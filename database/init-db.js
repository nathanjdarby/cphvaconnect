...
const insertSpeaker = db.prepare('INSERT INTO speakers (id, name, title, bio, photoUrl, company, linkedIn, twitter, website, isFeatured) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
speakers.forEach(speaker => {
  insertSpeaker.run(speaker.id, speaker.name, speaker.title, speaker.bio, speaker.photoUrl, speaker.company, speaker.linkedIn || null, speaker.twitter || null, speaker.website || null, speaker.isFeatured);
});
...
const insertLocation = db.prepare('INSERT INTO locations (id, name, capacity, description) VALUES (?, ?, ?, ?)');
locations.forEach(location => {
  insertLocation.run(location.id, location.name, location.capacity, location.description);
});
...
const events = [
  {
    ...
    description: 'Join us for an inspiring keynote on the future of healthcare delivery.',
...
const insertEvent = db.prepare('INSERT INTO schedule_events (id, title, description, startTime, endTime, locationId, eventType) VALUES (?, ?, ?, ?, ?, ?, ?)');
const insertEventSpeaker = db.prepare('INSERT INTO event_speakers (eventId, speakerId) VALUES (?, ?)');

events.forEach(event => {
  insertEvent.run(event.id, event.title, event.description, event.startTime, event.endTime, event.locationId, event.eventType);
...
const insertExhibitor = db.prepare('INSERT INTO exhibitors (id, name, description, logoUrl, websiteUrl, boothNumber) VALUES (?, ?, ?, ?, ?, ?)');
exhibitors.forEach(exhibitor => {
  insertExhibitor.run(exhibitor.id, exhibitor.name, exhibitor.description, exhibitor.logoUrl, exhibitor.websiteUrl, exhibitor.boothNumber);
});
...
const insertTicketType = db.prepare('INSERT INTO ticket_types (id, name, price, description, availableQuantity) VALUES (?, ?, ?, ?, ?)');
ticketTypes.forEach(type => {
  insertTicketType.run(type.id, type.name, type.price, type.description, type.availableQuantity);
});
...
const tickets = [
  {
    ...
    qrCodeValue: `TICKET-${Date.now()}-${Math.random().toString(36).substr(2, 9).toUpperCase()}`,
    purchaseDate: new Date().toISOString(),
    isCheckedIn: 0
  }
];

const insertTicket = db.prepare('INSERT INTO tickets (id, userId, ticketTypeId, qrCodeValue, purchaseDate, isCheckedIn) VALUES (?, ?, ?, ?, ?, ?)');
tickets.forEach(ticket => {
  insertTicket.run(ticket.id, ticket.userId, ticket.ticketTypeId, ticket.qrCodeValue, ticket.purchaseDate, ticket.isCheckedIn);
});
...
const insertPoll = db.prepare('INSERT INTO polls (id, question, isOpen) VALUES (?, ?, ?)');
const insertPollOption = db.prepare('INSERT INTO poll_options (id, pollId, text, votes) VALUES (?, ?, ?, ?)');
...
// Insert app settings
db.prepare('INSERT INTO app_settings (id, ticketSalesEnabled, registrationEnabled) VALUES (1, 1, 1)').run();
