...
export const ticketService = {
  ...
  async getAllTickets(): Promise<Ticket[]> {
    const db = getDatabase();
    const stmt = db.prepare(
      "SELECT * FROM tickets ORDER BY purchase_date DESC"
    );
    const rows = stmt.all() as any[];
...
export const scheduleService = {
  ...
  async updateEvent(
    ...
    const stmt = db.prepare(
      `UPDATE schedule_events SET ${fields
        .map((f) => `${f} = ?`)
        .join(", ")}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`
    );
    stmt.run(...values, id);
...
export const settingsService = {
  ...
  async updateAppSettings(settings: Partial<AppSettings>): Promise<void> {
    ...
    if (fields.length > 0) {
      const stmt = db.prepare(
        `UPDATE app_settings SET ${fields
          .map((f) => `${f} = ?`)
          .join(", ")}, updated_at = CURRENT_TIMESTAMP WHERE id = "settings"`
      );
      stmt.run(...values);
    }
...
export const ticketTypeService = {
  ...
  async updateTicketType(
    ...
    const stmt = db.prepare(
      `UPDATE ticket_types SET ${fields
        .map((f) => `${f} = ?`)
        .join(", ")}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`
    );
    stmt.run(...values, id);
