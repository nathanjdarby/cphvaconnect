...
export default function AdminExhibitorsPage() {
  ...
  const [exhibitorToEdit, setExhibitorToEdit] = useState<ExhibitorType | null>(
    null
  );
  const [isAlertOpen, setIsAlertOpen] = useState(false);
  const [exhibitorToDeleteId, setExhibitorToDeleteId] = useState<string | null>(
    null
  );
  ...
  const executeDeleteExhibitor = async () => {
    if (exhibitorToDeleteId && deleteExhibitor) {
      // Added check for deleteExhibitor
    ...
    <div className="space-y-6">
      <Card className="shadow-lg">
        <CardHeader className="flex flex-row items-center justify-between">
          <div className="space-y-1.5">
            ...
            <CardDescription>
              Add, edit, or delete conference exhibitors.
            </CardDescription>
          </div>
                  ...
                  <CardFooter className="border-t p-3 flex flex-col items-start space-y-2">
                    ...
                    <div className="flex justify-end gap-2 w-full">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => handleEditExhibitor(ex)}
                      >
                      ...
                      <Button
                        variant="destructive"
                        size="sm"
                        onClick={() => handleDeleteConfirmation(ex.id)}
                      >
      ...
      <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
        <SheetContent className="sm:max-w-xl overflow-y-auto">
          <SheetHeader>
            <SheetTitle>
              {exhibitorToEdit ? "Edit Exhibitor" : "Add New Exhibitor"}
            </SheetTitle>
            <SheetDescription>
              {exhibitorToEdit
                ? "Update the details for this exhibitor."
                : "Fill in the details for the new exhibitor."}
            </SheetDescription>
          </SheetHeader>
          <ExhibitorForm
            isOpen={isSheetOpen}
      ...
      <AlertDialog open={isAlertOpen} onOpenChange={setIsAlertOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={executeDeleteExhibitor}>
              Continue
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
