  ...
  const { checkedInTickets, pendingTickets, summary } = useMemo(() => {
    if (!allTickets)
      return {
        checkedInTickets: [],
        pendingTickets: [],
        summary: { total: 0, checkedIn: 0, pending: 0 },
      };

    const checkedIn = allTickets
      .filter((ticket) => ticket.isCheckedIn)
      .sort((a, b) => {
        if (a.checkInTimestamp && b.checkInTimestamp) {
          return (
            parseISO(b.checkInTimestamp).getTime() -
            parseISO(a.checkInTimestamp).getTime()
          );
        }
        return 0;
      });
    ...
    <div className="space-y-8">
      <Card className="shadow-xl">
        <CardHeader>
          ...
          <CardDescription>
            Overview of attendee check-in status.
          </CardDescription>
        </CardHeader>
          ...
          <div className="space-y-6">
            ...
            <section>
              <h2 className="text-2xl font-semibold text-primary/90 mb-3 flex items-center">
                ...
                Checked-In Attendees ({summary.checkedIn})
              </h2>
            ...
            <section>
              <h2 className="text-2xl font-semibold text-primary/90 mb-3 flex items-center">
                <XCircle className="mr-2 h-6 w-6 text-orange-500" /> Attendees
                Pending Check-In ({summary.pending})
              </h2>
              {pendingTickets.length === 0 ? (
                <p className="text-muted-foreground text-center py-4">
                  All ticket holders have checked in or no tickets sold.
                </p>
              ) : (
                <div className="overflow-x-auto border rounded-md">
                  <Table>
                    <TableCaption>
                      Attendees who have not yet checked in.
                    </TableCaption>
                    <TableHeader>
                      <TableRow>
                        <TableHead>Attendee Name</TableHead>
